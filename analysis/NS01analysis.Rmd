...
### Binary task
```{r Exclusions based on time on task: binary task, cache=T, echo=F}
...
### Continuous task
```{r Exclusions based on time on task: continuous task, cache=T, echo=FALSE}
...
choice2 <- glmer(recodedResponse ~ attention.difference*value.difference +
                (1 + attention.difference + value.difference||participantNo),
              data=data[task!="valuation",], family="binomial", 
              control=glmerControl(optimizer="Nelder_Mead"))
summary(choice2)

anova(choice2, choice1) # N.s so prefer choice2

choice3 <- glmer(recodedResponse ~ attention.difference + value.difference +
                (1 + attention.difference + value.difference||participantNo),
              data=data[task!="valuation",], family="binomial", 
              control=glmerControl(optimizer="Nelder_Mead"))
summary(choice3)
anova(choice2, choice3) # N.s so prefer choice3
# No interaction and no effect of task
```
...
### Analysing continuous task more fully
...
## Thoughts
- speed/accuracy trade off and it's relationship between interaction vs. not. 
...
- hopefully done wednesday so can forward to Tim/Neil :) 
...
A final intuition might be that participants were more accurate in the continuous task than the binary task. 