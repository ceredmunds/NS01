...
```
As preregistered, we excluded participants on the basis of time on task. Time on task was operationalized as the proportion of time during the trial where participants were fixating on either the pictures or the response scale. We then plotted a histogram of this to determine where a natural break point would be. For both tasks, this turned out to be 0.60. This resulted in excluding `r length(binaryExclusions) + length(contExclusions)` participants.
